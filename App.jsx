...
export default function App() {
  ...
  const handleCellPress = useCallback(async (row, col) => {
    ...
    try {
      ...
      // Update Firebase directly instead of using saveGameState
      if (firebaseRef.current) {
        await set(firebaseRef.current, {
          ...initialGameState,
          tokens: newTokens,
  ...
  }, [tokens, currentColor, layers, initiative, inCombat, currentTurn, partyLoot, isUpdating]);
                      ...
                      {DICE_TYPES.map(({ sides, color }) => (
                        <View key={sides} style={diceStyles.diceContainer}>
                          <DiceRoller
                            ...
                            onRoll={(newResults) => {
                              // Use the results directly from the dice roll
